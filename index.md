...
<h1>Welcome to the Feminist Movie Game</h1>

<p>Read the movie summary and decide if it's feminist or not.</p>

<!-- Buttons for user to select their guess -->
<button id="feministButton">Feminist</button>
<button id="nonFeministButton">Not Feminist</button>

<div id="movieSummary">
  <!-- Movie Summary will be displayed here -->
</div>
...
fetch('{{ "/assets/plots/movies_summaries_feminism.json" | relative_url }}')
  ...
  .then(movies => {
    ...
    // Function to pick a random movie summary
    function pickRandomMovie() {
      const randomIndex = Math.floor(Math.random() * movies.length);
      const movie = movies[randomIndex];

      // Set the correct answer based on the feminism response
      correctAnswer = movie['feminism response'] === 1 ? 'feminist' : 'not feminist';
    ...
    // Pick a random movie summary when the page loads
    pickRandomMovie();

    // Event listener for the "Feminist" button
    document.getElementById('feministButton').addEventListener('click', function() {
      checkAnswer('feminist');
    });

    // Event listener for the "Not Feminist" button
    document.getElementById('nonFeministButton').addEventListener('click', function() {
      checkAnswer('not feminist');
    });
    ...
    function checkAnswer(userGuess) {
      ...
      if (userGuess === correctAnswer) {
        gameResult.innerHTML = "Correct! Well done.";
        score += 1; // Increase score for correct guess
      } else {
        gameResult.innerHTML = `Wrong! The correct answer is ${correctAnswer}.`;
      }

      // Update the score display
      document.getElementById('score').innerHTML = `Score: ${score}`;

      // After the guess, pick a new movie summary
      pickRandomMovie();
    }
  })
  .catch(error => console.log('Error loading movie data:', error));
